...
create table forum_example.job (
...
comment on table forum_example.job is 'a job.';
comment on column forum_example.job.id is 'the primary unique identifier for the job.';
comment on column forum_example.job.geom is 'the job’s geospatial location.';
comment on column forum_example.job.category is 'the job’s category.';
comment on column forum_example.job.created_at is 'The time this job was created.';
comment on column forum_example.job.updated_at is 'The time this job was updated.';

-- add a spatial index
create index job_gix
  on forum_example.job
  using gist (geom);

create function forum_example.search_jobs(search integer) returns setof forum_example.job as $$
  select job.*
  from forum_example.job as job
  -- where job.headline ilike ('%' || search || '%') or post.body ilike ('%' || search || '%')
  where st_dwithin(
    geom,
    st_geomfromtext('point(0 0)', search),
    1000
  );
$$ language sql stable;
...
grant select on table forum_example.job to forum_example_anonymous, forum_example_person;
grant update, delete on table forum_example.job to forum_example_person;
...
alter table forum_example.job enable row level security;
...
create policy select_job on forum_example.job for select
  using (true);
